...
# 如ps、top、lsof、netstat、ifconfig、who、df、kill、free、nice、ionice、iostat、iotop、
# uptime、pidof、tty、taskset、pmap等。目前支持32位和64位的Linux、Windows、OS X、FreeBSD
# 和Sun Solaris等操作系统.
import psutil
...
if __name__ =="__main__":
    for i in range(30):
        ...
        # 访问 video 文件夹
        # root指的是当前正在遍历的这个文件夹的本身的地址，dirs是一个 list，内容是该文件夹中所有的目录的名字
        # (不包括子目录)，files同样是 list，内容是该文件夹中所有的文件(不包括子目录)
        ...
        # 法二：最常规的生成目标视频文件方法
        final_clip.write_videofile(targetdir,fps=24,remove_temp=True) #remove_temp=True表示生成的音频文件是临时存
        # 放的，视频生成后，音频文件会自动处理掉！若为False表示，音频文件会同时生成！
